...
const main = async () => {
  ...
  const program = new Program<CastleLendingAggregator>(
    vaultIdl as CastleLendingAggregator,
    VAULT_PROGRAM_ID,
    provider
  );
  ...
  const solendReserve = solendMarket.reserves.find(
    (res) => res.config.symbol === "SOL"
  );
  const solendCollateralMint = new PublicKey(
    solendReserve.config.collateralMintAddress
  );

  const port = new Port(
    connection,
    Environment.forMainNet(),
    new PublicKey("H27Quk3DSbu55T4dCr1NddTTSAezXwHU67FPCZVKLhSW")
  );
  const portReserve = await port.getReserve(
    new PublicKey("6FeVStQAGPWvfWijDHF7cTWRCi7He6vTT3ubfNhe9SPt")
  );

  const jetClient = await JetClient.connect(provider, true);
  const jetReserve = await JetReserve.load(
    jetClient,
    new PublicKey("BrXRUKDaSnxHwL46J4LSPiHnundhLzTfKRToh4s9jFbK")
  );
  ...
  const [vaultJetLpTokenAccount, jetLpBump] = await PublicKey.findProgramAddress(
    [
      ...
      jetReserve.data.depositNoteMint.toBuffer(),
    ],
    program.programId
  );
